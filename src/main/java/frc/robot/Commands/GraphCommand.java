...
/**
 * Executes {@link Command}s along a directed graph. This utility allows complex mechanisms to move
 * through a defined sequence of waypoints while still reacting to new targets the driver selects.
 */
public class GraphCommand extends Command {
  ...
  /**
   * Set's the root of the graph. All there must be a way to get from the root node to every other
   * node in the graph. It can take zero, one or more waypoints to reach each node.
   *
  ...
  public class GraphCommandNode {
     ...
     * @param waypointCommand - the command to run if the node is a waypoint (not the final target)
     * @param arrivedCommand - the command to run when the graph is at the node and the node was the
     *     target
     */
    public GraphCommandNode(
        String nodeName, Command targetCommand, Command waypointCommand, Command arrivedCommand) {
      m_nodeName = nodeName;
     ...
     * @return The cost of traveling from this node to the target after optimization. If there is no
     *     path, {@code Double.MAX_VALUE} is returned.
     */
    ...
    public GraphCommandNode getNextNodeGivenTarget(GraphCommandNode node) {
      ...
      DataLogManager.log(
          "GraphCommand next node for: "
              + node.getNodeName()
              + " is "
              + link.m_wayPointNode.getNodeName());
    ...
    public void optimizeGraph(
        ...
        GraphCommandNode parentNode,
        Map<String, GraphCommandNodeLink> bestLinks,
        double cost) {
      ...
      while (iterator.hasNext()) {
        ...
        neighborLink.m_node.optimizeGraph(
            rootNode, parentNode, bestLinks, neighborLink.m_cost + cost);
      }
     ...
     * @param cost - the cost of using this node as an intermediate node. Must be positive number.
     * @param isOneWay - Default value is false and it will add both directions with equal cost.
     * @return
     */
     ...
     * Adds a node as a child to a node.
     *
     * @param childNode - Node to connect. Node cannot be connected to itself. This method creates a
     *     bi-directional link with equal cost.
     * @param cost - the cost of using this node as an intermediate node. Must be positive number.
     * @return
     */
